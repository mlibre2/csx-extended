...
static cell AMX_NATIVE_CALL get_user_wrstats(AMX *amx, cell *params) /* 4 param */
{
	int index = params[1];
	CHECK_PLAYERRANGE(index);
	int weapon = params[2];
...
static cell AMX_NATIVE_CALL set_user_stats(AMX *amx, cell *params) /* 3 param */ /*Added IN CSX Extended*/
{
	int index = params[1];
	CHECK_PLAYER(index);
	CPlayer* pPlayer = GET_PLAYER_POINTER_I(index);
...
static cell AMX_NATIVE_CALL set_user_stats2(AMX *amx, cell *params) /* 2 param */ /*Added IN CSX Extended*/
{
	int index = params[1];
	CHECK_PLAYER(index);
	CPlayer* pPlayer = GET_PLAYER_POINTER_I(index);
...
static cell AMX_NATIVE_CALL set_user_score(AMX *amx, cell *params)  /*3 param */ /*Added IN CSX Extended*/
{
	int index = params[1];
	CHECK_PLAYER(index);
	CPlayer* pPlayer = GET_PLAYER_POINTER_I(index);
	pPlayer->restartStats();
	pPlayer->setScore(params[1],params[2]);
	return 1;
}
...
static cell AMX_NATIVE_CALL get_user_rank(AMX *amx, cell *params) /* 1 param */ /*Added IN CSX Extended*/
{
	int index = params[1];
	CHECK_PLAYERRANGE(index);
	return GET_PLAYER_POINTER_I(index)->rank->getPosition();
}

static cell AMX_NATIVE_CALL set_stats(AMX *amx, cell *params) /* 3 param */ /*Added IN CSX Extended*/
{
	int index = params[1] + 1;
	RankSystem::iterator a;
	a.setEntryByRank( index );
...
static cell AMX_NATIVE_CALL set_stats2(AMX *amx, cell *params) /* 2 param */ /*Added IN CSX Extended*/
{
	int index = params[1] + 1;
	RankSystem::iterator a;
	a.setEntryByRank( index );
...
static cell AMX_NATIVE_CALL reset_user_stats(AMX *amx, cell *params)  /*1 param */ /*Added IN CSX Extended*/
{
    int index = params[1];
    CHECK_PLAYER(index);
    CPlayer* pPlayer = GET_PLAYER_POINTER_I(index);
    if( pPlayer->rank )
	{
		...
		pPlayer->setScore(0,0);
        return pPlayer->rank->getPosition();
    }
    return 0;
}

static cell AMX_NATIVE_CALL reset_stats(AMX *amx, cell *params) /* 1 param */ /*Added IN CSX Extended*/
{
	int index = params[1] + 1;
	RankSystem::iterator a;
	a.setEntryByRank( index );
...
static cell AMX_NATIVE_CALL remove_stats(AMX *amx, cell *params) /* 1 param */ /*Added IN CSX Extended*/
{
	int index = params[1] + 1;
	RankSystem::iterator a;
	a.setEntryByRank( index );
	...
	if (a)
	{
		for (int i=1;i<=gpGlobals->maxClients;i++)
		{
			...
			{
				(*a).deletePosition();
				GET_PLAYER_POINTER_I(i)->PutInServer();
				GET_PLAYER_POINTER_I(i)->setScore(0,0);
				return 1;
			}
		}
		(*a).deletePosition();
...
static cell AMX_NATIVE_CALL push_stats(AMX *amx, cell *params) /* 5 param */ /*Added IN CSX Extended*/
{
	int iLen;
	char* Unique = MF_GetAmxString(amx, params[1], 0, &iLen);
	char* Name = MF_GetAmxString(amx, params[2], 0, &iLen);
	RankSystem::RankStats* temp = g_rank.findEntryInRank( Unique, Name );
	if ( temp )
	{
...
static cell AMX_NATIVE_CALL custom_wpn_dmg(AMX *amx, cell *params)
{
	...
	if ( (pVic->teamId == pAtt->teamId) && ( pVic != pAtt) )
		TA = 1;
	MF_ExecuteForward( iFDamage, static_cast<cell>(pAtt->index), static_cast<cell>(pVic->index), static_cast<cell>(dmg), static_cast<cell>(weapon), static_cast<cell>(aim), static_cast<cell>(TA) );
	if ( pVic->IsAlive() )
		return 1;
	pAtt->saveKill(pVic,weapon,( aim == 1 ) ? 1:0 ,TA);
	MF_ExecuteForward( iFDeath, static_cast<cell>(pAtt->index), static_cast<cell>(pVic->index), static_cast<cell>(weapon), static_cast<cell>(aim), static_cast<cell>(TA) );
	return 1;
}
...
static cell AMX_NATIVE_CALL force_load_stats(AMX *amx, cell *params) /* no param */ /*Added IN CSX Extended*/
{
